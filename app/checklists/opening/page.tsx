...
import { OPENING_CHECKLIST, getDayOfWeekName, getSideworkTask } from '@/lib/shift-checklists'
...
export default function OpeningChecklistPage() {
  ...
  const totalCount = OPENING_CHECKLIST.items.length
  const progress = (completedCount / totalCount) * 100

  const sidework = getSideworkTask('opening', dayOfWeek)
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="display-md mb-2">Opening Checklist</h1>
      ...
      <div className="space-y-3 mb-8">
        {OPENING_CHECKLIST.items.map((item) => {
          const isChecked = checkedItems.has(item.id)
          return (
            <div
              key={item.id}
              onClick={() => !item.isSection && toggleItem(item.id)}
              className={`card-sm flex items-center gap-4 cursor-pointer transition-colors ${
                item.isSection ? 'bg-agave-900/20 font-medium text-agave-300' : 'hover:bg-ink-700'
              } ${isChecked && !item.isSection ? 'bg-agave-900/30' : ''}`}
            >
              {!item.isSection && (
                <>
                  <button
